...
def display_battle_tab():
    """Enhanced battle mode without database changes"""
    if not st.session_state.student_name:
        st.info("Please enter your name in the sidebar to start a battle!")
        return
    ...
    if 'battle_timer' not in st.session_state:
        st.session_state.battle_timer = 15
    ...
    if not st.session_state.battle_mode:
        # Create or Join Battle UI
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("### 🎮 Create Battle Room")
            with st.container():
                st.markdown("""
                🏆 Battle Features:
                - Real-time gameplay
        ...
        with col2:
            st.markdown("### 🤝 Join Battle")
            room_code = st.text_input("Enter room code:")
            if room_code and st.button("⚔️ Join Battle", type="primary"):
        ...
        if battle_info:
            ...
            with col1:
                st.code(f"Room: {st.session_state.battle_id}", language=None)
            with col2:
                st.markdown(f"**Category:** {battle_info['category']}")
            with col3:
            ...
            if battle_info['status'] == 'waiting':
                ...
                st.markdown("### 🎲 Share this code with your opponent!")
                
                # Practice Mode
                st.divider()
                st.markdown("### 🎯 Practice Round")
                st.markdown("While waiting, try these practice questions!")
                display_practice_questions(battle_info['questions'])
                
            elif battle_info['status'] == 'in_progress':
                # Score Display
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Your Score", 
                    ...
                    if st.session_state.current_streak > 1:
                        st.markdown(f"🔥 Streak: {st.session_state.current_streak}")
                with col2:
                    opponent_name = battle_info['joiner'] if st.session_state.student_name == battle_info['creator'] else battle_info['creator']
                    st.metric(f"{opponent_name}'s Score",
                             battle_info['joiner_score'] if st.session_state.student_name == battle_info['creator'] else battle_info['creator_score'])
                
                # Timer
                current_time = datetime.now()
                elapsed = (current_time - st.session_state.last_update).seconds
                st.session_state.battle_timer = max(0, st.session_state.battle_timer - elapsed)
                st.session_state.last_update = current_time
                
                progress = st.session_state.battle_timer / 15
                st.progress(progress)
                st.markdown(f"⏱️ Time remaining: **{st.session_state.battle_timer}s**")
                
                # Question Display
                if not st.session_state.battle_submitted:
                    display_battle_questions(battle_info)
                
                if st.session_state.battle_timer <= 0:
                    st.warning("⌛ Time's up!")
                    submit_current_answer(battle_info)
                    st.session_state.battle_timer = 15
                    st.rerun()

def display_battle_questions(battle_info):
    """Display battle questions with enhanced UI"""
    if not st.session_state.battle_questions:
        return
    
    current_q = st.session_state.battle_questions[st.session_state.current_question_index]
    
    st.markdown(f"### Question {st.session_state.current_question_index + 1}")
    st.markdown(f"**{current_q['question']}**")
    
    # Create a grid of answer buttons
    cols = st.columns(2)
    for i, option in enumerate(current_q['options']):
        with cols[i % 2]:
            if st.button(
                f"{option}",
                key=f"battle_option_{i}",
                use_container_width=True,
                type="primary"
            ):
                ...
                submit_answer(battle_info, is_correct)
                st.session_state.battle_timer = 15
                st.rerun()


def display_practice_questions(questions):
    """Show practice questions while waiting"""
    if not questions:
        return
    
    for i, q in enumerate(questions[:2]):  # Show only 2 practice questions
        ...
        if answer:
            if answer == q['answer']:
                st.success("✅ Correct! Ready for battle!")
            else:
                st.error("❌ Try again! Practice makes perfect!")

def submit_answer(battle_info, is_correct):
    ...
    try:
        ...
        supabase.table('battle_rooms').update({
            field: new_score
        }).eq('id', battle_info['id']).execute()
        
        # Move to next question
        st.session_state.current_question_index += 1
        if st.session_state.current_question_index >= len(st.session_state.battle_questions):
            complete_battle(battle_info['id'])
            
    except Exception as e:
        st.error(f"Error submitting answer: {str(e)}")

def submit_current_answer(battle_info):
    """Submit current answer when timer runs out"""
    submit_answer(battle_info, False)  # Count as incorrect when time runs out
    st.session_state.current_streak = 0

def complete_battle(room_id):
    """Complete the battle when all questions are answered"""
    try:
        supabase.table('battle_rooms').update({
            'status': 'completed'
        ...
        st.error(f"Error completing battle: {str(e)}")
